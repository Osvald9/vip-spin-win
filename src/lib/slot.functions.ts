...
export const claimMemoryPrize = createServerFn({ method: "POST" })
  .inputValidator(
    (data: unknown) =>
      z
        .object({
          participantId: z.string().uuid(),
          level: z.enum(["facil", "medio", "dificil"]),
          won: z.boolean(),
        })
        .parse(data)
  )
  .handler(async ({ data }) => {
    if (shouldUseSupabase()) {
      ...
      const caneta = pool.find(p => p.name.toLowerCase().includes("caneta"));
      const copoPlastico = pool.find(p => p.name.toLowerCase().includes("copo plastico") || p.name.toLowerCase().includes("copo plástico"));
      const copoTermico = pool.find(p => p.name.toLowerCase().includes("termico") || p.name.toLowerCase().includes("térmico") || p.name.toLowerCase().includes("staley") || p.name.toLowerCase().includes("stanley"));

      let winner: (typeof pool)[number] | null = null;

      if (data.level === "facil") {
        const targets = [caneta, copoPlastico].filter((x): x is typeof pool[number] => !!x);
        if (targets.length > 0 && Math.random() < 0.90) {
          winner = targets[Math.floor(Math.random() * targets.length)];
        }
      } else if (data.level === "medio") {
        const targets = [caneta, copoPlastico, copoTermico].filter((x): x is typeof pool[number] => !!x);
        if (targets.length > 0) {
          winner = targets[Math.floor(Math.random() * targets.length)];
        }
      } else if (data.level === "dificil") {
        if (copoTermico && Math.random() < 0.70) {
          winner = copoTermico;
        }
      }
      ...
      const caneta = pool.find((p: any) => p.name.toLowerCase().includes("caneta"));
      const copoPlastico = pool.find((p: any) => p.name.toLowerCase().includes("copo plastico") || p.name.toLowerCase().includes("copo plástico"));
      const copoTermico = pool.find((p: any) => p.name.toLowerCase().includes("termico") || p.name.toLowerCase().includes("térmico") || p.name.toLowerCase().includes("staley") || p.name.toLowerCase().includes("stanley"));

      let winner: any = null;

      if (data.level === "facil") {
        const targets = [caneta, copoPlastico].filter(Boolean);
        if (targets.length > 0 && Math.random() < 0.90) {
          winner = targets[Math.floor(Math.random() * targets.length)];
        }
      } else if (data.level === "medio") {
        const targets = [caneta, copoPlastico, copoTermico].filter(Boolean);
        if (targets.length > 0) {
          winner = targets[Math.floor(Math.random() * targets.length)];
        }
      } else if (data.level === "dificil") {

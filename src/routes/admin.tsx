    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="mx-auto max-w-6xl px-6 py-8">
        {loading ? (
          ...
          <PrizesTab prizes={prizes} onSave={save} onDelete={remove} />
        ) : (
  ...
  return (
    <div className="grid gap-3 sm:grid-cols-[auto_1fr_auto_auto_auto_auto_auto]">
      ...
      <div className="flex gap-2">
        <button
          onClick={() => onSave()}
          className="btn-vip btn-vip-hover flex items-center gap-1 rounded-lg px-3 py-2 text-sm"
        >
        ...
        {onDelete && (
          <button
            onClick={onDelete}
            className="rounded-lg border border-destructive/50 bg-destructive/10 px-3 py-2 text-destructive"
          >
      ...
      <div className="col-span-full flex items-center gap-3 text-xs text-muted-foreground">
        <SlotIcon name={prize.icon} className="h-5 w-5 text-primary" />
        <span>
